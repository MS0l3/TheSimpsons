...
const buildEpisodeInfo = (seasonNumber, episodeCode) => ({
  titulo: `Episodio ${episodeCode} · Temporada ${seasonNumber}`,
  descripcion:
    `Ficha de capítulo ${episodeCode}. Aquí podrás mostrar sinopsis real, duración y otros metadatos. ` +
    'De momento es un texto de ejemplo para la vista de detalle.',
});
...
export default function App() {
  ...
  const episodioInfo = useMemo(() => {
    if (!selectedEpisode || !temporadaActiva) {
      return null;
    }

    return buildEpisodeInfo(temporadaActiva.temporada, selectedEpisode.codigo);
  }, [selectedEpisode, temporadaActiva]);
